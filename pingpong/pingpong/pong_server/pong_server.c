...
/* TO BE DONE START */
		if(clock_gettime(CLOCK_TYPE,&time2)==-1) 
			fail("error during time-stamp time2");
/* TO BE DONE END */
...
/* TO BE DONE START */
		if(clock_gettime(CLOCK_TYPE,&time3)==-1)
			fail("error during time-stamp time3");
...
/* TO BE DONE START */
			if(clock_gettime(CLOCK_TYPE,&time2)==-1)
				fail("error during time-stamp 2");
...
/* TO BE DONE START */
		if(clock_gettime(CLOCK_TYPE,&time3))
				fail("error during time-stamp time 3");
...
/* TO BE DONE START */
	if(gai_rv=getaddrinfo(NULL,port_number_as_str,&gai_hints,&pong_addrinfo)<0)
		fail("error during getaddrinfo to set port number");
	if((udp_socket=socket(gai_hints.ai_family,gai_hints.ai_socktype,gai_hints.ai_protocol))<0)
		fail("error during creating DGRAM socket");
...
/* TO BE DONE START */
	if(getaddrinfo(NULL,argv[1],&gai_hints,&server_addrinfo)<0)
		fail("error during gettadrinfo to set upt port number and server adress");
	if((server_socket=socket(AF_INET, SOCK_STREAM, IPPROTO_TCP))<0)
		fail("error during the creation of STREAM socket");
	if(bind(server_socket,server_addrinfo->ai_addr,server_addrinfo->ai_addrlen)<0)
		fail("error during the bind");
	if(listen(server_socket,LISTENBACKLOG)==-1)
		fail("error during the listen() function");
