 ...
 * in Informatica classe L-30 presso l'Universita` degli Studi di
...
/*** TO BE DONE START ***/
	...
	if(sprintf(message, "%d", msg_no) < 0)
		fail_errno("errore nel caricamento del messaggio nel buffer");
...
/*** TO BE DONE START ***/
	...
	if(clock_gettime(CLOCK_REALTIME, &send_time) < 0)
		fail_errno("errore nel salvataggio del tempo");
...
/*** TO BE DONE START ***/
	...
	if(sent_bytes < 0)
		fail("errore nel'inviare i byte (modalità bloccante)");
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***/
	...
	if(getaddrinfo(argv[1], argv[2], &gai_hints, &server_addrinfo) != 0)
		fail_errno("errore nella getaddrinfo");
...
/*** TO BE DONE START ***/
	...
	if(blocking_write_all(tcp_socket, request, strlen(request)) < 0)
                fail("errore nell'inviare la richiesta");
...
/*** TO BE DONE START ***/
	...
	if(strncmp(answer, "OK\n",nr) != 0)
		fail_errno("risposta dal server non valida");
