...
/*** TO BE DONE 6.0 START ***/
	...
	if(gmtime_r(&now_t,&now_tm) == NULL)
		fail_errno("error during compute date of servicing current HTTP Request");
...
/*** TO BE DONE 6.0 START ***/
			...
			sprintf(http_header + strlen(http_header),"\r", cookie);
...
/*** TO BE DONE 6.0 START ***/
	...
	if(sendfile(client_fd,fd,NULL,file_size) == -1)
		fail_errno("error in the send file");
	if(close(fd) == -1)
		fail_errno("error in the close");
...
/*** TO BE DONE 6.0 START ***/
					...
					strtok(http_option_line, " \t");
					if(strcmp(option_name,"If-Modified-Since")!=0)
						perror("error during the parse");
					else{
						http_method = METHOD_CONDITIONAL;
						strftime(strtokr_save, sizeof(strtokr_save),"%A" "%D" "%B" "%Y", &since_tm);
					}	
