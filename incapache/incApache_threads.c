...
/*** TO BE DONE 6.1 START ***/
	...
	if(to_join[conn_no] != NULL){
		i = MAX_CONNECTIONS;
		while((i < MAX_THREADS) && (to_join[conn_no] != &thread_ids[i])) i++;
		
		if(pthread_join(thread_ids[i], NULL) != 0)
			fail_errno("errore nella pthread_join");
...
/*** TO BE DONE 6.1 START ***/
	...
	{
		i = 0;
		while (i < MAX_THREADS && to_join[thrd_no] != &thread_ids[i]) i++;
		if(pthread_join(thread_ids[i], NULL) != 0)
			fail_errno("errore nella pthread_join");
		...
		if(i >= MAX_CONNECTIONS)
		{
			...
			if(pthread_mutex_lock(&threads_mutex) != 0)
				fail_errno("errore nella lock di threads_mutex");
			no_response_threads[conn_no];
			if(no_response_threads[conn_no] == 0)
				no_free_threads++;
			connection_no[i] = FREE_SLOT;
			if(pthread_mutex_unlock(&threads_mutex) != 0)
				fail_errno("errore nella unlock di threads_mutex");
		}
	}
...
/*** TO BE DONE 6.0 START ***/
	if(pthread_mutex_lock(&mime_mutex) != 0)
		fail_errno("error during the locking of mime_mutex");
...
/*** TO BE DONE 6.0 START ***/
	if(pthread_mutex_unlock(&mime_mutex) != 0)
		fail_errno("error during the unlocking of mime_mutex");
