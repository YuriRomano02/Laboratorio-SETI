...
check_t check_redirections(const line_t * const l)
{
	 ...
	 */
	/*** TO BE DONE START ***/
	for(int i = 0; i < l->n_commands && l->n_commands > 1; i++){
		if(i != 0 && l->commands[i]->in_pathname != 0){
			perror("La redirezione in input può averla solo il primo comando");
			return CHECK_FAILED;
		}
		if(i != l->n_commands-1 && l->commands[i]->out_pathname != 0){
			perror("La redirezione in output può averla solo l'ultimo comando");
			return CHECK_FAILED;
		}
	}
	/*** TO BE DONE END ***/
...
check_t check_cd(const line_t * const l)
{
	...
	for(int i = 0; i < l->n_commands; i++){
		...
		if (strcmp(CD, l->commands[i]->args[0]) == 0){
			if(l->n_commands != 1){
				printf("Errore: cd deve essere l'unico comando nella linea\n");
				flag = 1;
			}
			if((l->commands[i]->in_pathname != 0 ) || (l->commands[i]->out_pathname != 0)){
				printf("Errore: cd non deve avere redirezioni di I/O\n");
				flag = 1;
			}
			if(l->commands[i]->n_args != 2){
				printf("Errore: cd deve avere un solo argomento\n");
				flag = 1;
			}
			if(flag) 
				return CHECK_FAILED;
...
void run_child(const command_t * const c, int c_stdin, int c_stdout)
{
	...
	if(p == 0){
		...
		if(execvp(c->args[0], c->args) < 0)
			perror("errore nell'eseguire il comando");
	}
	/*** TO BE DONE END ***/
}
...
void change_current_directory(char *newdir)
{
	...
	/*** TO BE DONE START ***/
	if(chdir(newdir) != 0)
		perror("Errore nel cambio di cartella");
	/*** TO BE DONE END ***/
}
...
void execute_line(const line_t * const l)
{
	...
	for(int a=0; a<l->n_commands; ++a) {
		...
		if (c->in_pathname) {
			...
			if(curr_stdin == -1)
				perror("Errore nella open del c->in_pathname");
			/*** TO BE DONE END ***/
		}
		if (c->out_pathname) {
			...
			if(curr_stdout == -1)
				perror("Errore nella open del c->out_pathname");
			/*** TO BE DONE END ***/
			...
			if(fcntl(fds[0], F_SETFD, FD_CLOEXEC))
				fatal_errno("Errore nell'assegnazione di FD_CLOEXEC su fds[0]");
			if(fcntl(fds[1], F_SETFD, FD_CLOEXEC))
				fatal_errno("Errore nell'assegnazione di FD_CLOEXEC su fds[1]");
			/*** TO BE DONE END ***/
			curr_stdout = fds[1];
			next_stdin = fds[0];
		}
...
int main()
{
	...
	for(;;) {
		...
		/*** TO BE DONE START ***/
		pwd = getcwd(NULL, 0);
		if(!pwd) fatal_errno("Errore nella getcwd");
		/*** TO BE DONE END ***/
